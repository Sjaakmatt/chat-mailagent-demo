/**
 * Het detailscherm van een klantenservice-item.
 *
 * Stond tot fase 4 als pagina onder `ui/app/(dashboard)/mail/[id]/`, met de
 * losse onderdelen in `ui/components/mail-detail/`. Allebei schil-mappen, en
 * allebei vol met mailkennis: de originele mail, de thread, de bijlagen, het
 * concept-antwoord. Een tweede module kon daar niets mee, en een derde zou er
 * een `if` bij hebben gekregen.
 *
 * Nu levert de module zijn eigen detailweergave en levert de schil de
 * omlijsting: de route (`/item/[id]`), de rechtencontrole, en de assistent in
 * de zijkant. Zie de toelichting bij `DetailView` in `ui/lib/modules/contract.ts`
 * voor waarom de "een module levert geen React"-regel hier bewust niet geldt.
 *
 * De rij komt van de route; alles wat dít scherm er verder bij nodig heeft —
 * de edit-historie, het beslislog, de klaargezette acties, het ticket, de
 * ondertekende bijlage-links — haalt het zelf op. Welke aanvullingen dat zijn,
 * weet alleen de module.
 */
...
import { mailProposed } from "../index";
...
export async function KlantenserviceDetail({
  row,
}: {
  row: ReviewItemRow;
  user: AuthedAccess;
}) {
  // De omgeving voor de aanvullingen die dít scherm erbij haalt. Fail-soft en
  // apart van de rij: valt de binding weg, dan hoort er nog steeds een voorstel
  // te staan — zonder tijdlijn, niet zonder scherm.
